...
install_version() {
  ...
  (
    ...
    # Install the package
    "$install_path/venv/bin/pip" install --upgrade pip setuptools wheel || fail "Could not upgrade pip"
    "$install_path/venv/bin/pip" install "$extracted_dir" || fail "Could not install ${TOOL_NAME}"
